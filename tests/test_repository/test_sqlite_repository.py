...
def test_crud(repo, test_class):
    obj = test_class("Alice")
    pk = repo.add(obj)
    assert obj.pk == pk
    assert repo.get(pk) == obj
    obj2 = test_class("Bob", pk)
    repo.update(obj2)
...
def test_get_all_filters_objects(repo, test_class):
    obj1 = test_class("Alice", 1)
    obj2 = test_class("Bob", 2)
    obj3 = test_class("Alice", 3)
    repo.add(obj1)
    repo.add(obj2)
    repo.add(obj3)
    objs = repo.get_all(where={"name": "Alice"})
    assert obj1 in objs
